...
% robot = getRobot();
% currentConfig = [0 pi/2 0 pi/2 0];
%Ask for a target pose
x = 0; y = -0.207; z = -0.0650; phi = 3*pi/2;

sol = findSolution(x, y, z, phi, currentConfig(1:4)', robot);

if ~isempty(sol)
    disp('Optimal DH solution (rad):'); disp(sol);
    disp('Optimal DH solution (deg):'); disp(sol * 180/pi);
else
