function success = executePipeline(robot, x1,y1,z1,phi1, x2,y2,z2,phi2)
    ...
    success = false;
    HOVER_HEIGHT = 0.07;  % how high above target to hover before descending
    ...
    % currentConfig = arb.getpos();
    % arb.setpos(5, pi/2.7, 100);
    moveToConfig(arb, pi/2.7, grasp_q, 80);
    ...
    moveToConfig(arb,pi/2.7, hover_q, 80);
    pause(7);
    ...
    moveToConfig(arb, pi/2.7,place_hover_q, 80);
    pause(7);
    ...
    moveToConfig(arb, pi/2.7, place_q, 80);
    pause(2.5);
    
    %% phase 8 - release
    disp('Phase 8: Releasing object...');
    moveToConfig(arb, 0, place_q, 80);
    pause(2.5);
